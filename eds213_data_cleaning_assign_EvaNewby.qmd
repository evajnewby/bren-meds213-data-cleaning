...
### 3. Use the relationship between the three cover columns (Snow, Water, Land) to infer missing values where possible and recompute the Total_cover column as needed **need to review, all below from Claude AI**
...
```{r}
# Recompute Total_cover based on component values
landsurvey_fixed <- landsurvey_fixed %>%
...
There are still some Total_cover values that are less than 100, and they all contain a row with only the "Snow_cover data". As there is no specification in the metadata about what to do with NA values in both Water_cover and Land_cover, I will...
- remove the rows?
- convert the Snow_cover in this column to NAs?
- or split the remaining totals between the two columns? IDK, time for bed and will figure out tomorrow. 
```{r}
